...
let is_cell_already_shot (size: int) (cell_map: board_cell_type CoordMap.t) (x: int) (y: int) : bool =
  match is_coordinate_invalid size x y with
  | true -> true
  | false ->
...
let alive_ship_coordinates (board : Battleship_types.board) : Coord.t list =
  let extract ship = List.map (fun c -> (c.x_coordinate, c.y_coordinate)) ship.coordinates in
...
let is_ship_alive_hit (cell_map: board_cell_type CoordMap.t) (alive_coords: Coord.t list) (x: int) (y: int) : bool =
  match not (List.mem (x, y) alive_coords) with
  | true -> false
  | false ->
    match CoordMap.find_opt (x, y) cell_map with
    | Some Hit -> true
  ...
  let rec iterate_rows y acc =
    ...
    | false ->
      let rec iterate_cols x row =
        match x > size with
        | true -> row
        | false -> iterate_cols (x + 1) ((x, y) :: row)
      in
    ...
    | false ->
      List.fold_left
        (fun acc_map (dx, dy) ->
          let nx = x + dx in
          let ny = y + dy in
          match is_cell_already_shot size cell_map nx ny with
          | true -> acc_map
          | false ->
            let bonus =
...
let hard_next_fire_coordinate (board : Battleship_types.board) : Battleship_types.coordinate =
  ...
  let best =
    CoordMap.fold (fun (x,y) value acc ->
      ...
      | None -> Some (value, [(x,y)])
      | Some (max_v, coords) ->
          match value > max_v with
          | true -> Some (value, [(x,y)])
          | false ->
            match value = max_v with
            | true -> Some (max_v, (x,y)::coords)
            | false -> acc
    ) prob_map None
  in
  match best with
